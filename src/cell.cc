...
/**
 * This is the legion engines custom function for serializing the voronoi data
 * Outputs the edges of the Voronoi cell in a JSON format to an open file
 * stream, displacing the cell by given vector.
 * \param[in] (x,y,z) a displacement vector to be added to the cell's position.
 * \param[in] fp a file handle to write to. */
void voronoicell_base::draw_json(double x, double y, double z,double cellId, FILE* fp) {
	int i, j, k; double* ptsp = pts, * pt2;
	char posbuf1[128], posbuf2[128];
	for (i = 0; i < p; i++, ptsp += 3) {
		sprintf(posbuf1, "\"x\": %g,\"y\": %g,\"z\": %g,\"w\": %g", x + *ptsp * 0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5,cellId);
		//fprintf(fp, "\t\"Point\":\n\t{\n%s\n\t},\n", posbuf1);
		for (j = 0; j < nu[i]; j++) {
			k = ed[i][j];
			if (k < i) {
				pt2 = pts + 3 * k;
				sprintf(posbuf2, "\"x\": %g,\"y\": %g,\"z\": %g,\"w\": %g", x + *pt2 * 0.5, y + 0.5 * pt2[1], z + 0.5 * pt2[2],cellId);
				if (strcmp(posbuf1, posbuf2) != 0) fprintf(fp, "\n[ {%s},{%s}],", posbuf1, posbuf2);
			}
		}
